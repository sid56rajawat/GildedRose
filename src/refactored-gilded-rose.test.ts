import { Item, GildedRose, ConjuredItem} from './refactored-gilded-rose';
...
describe('Tests for Conjured Items', () => {
    test('Conjured Item degrades twice as fast as normal Item', () => {
        const gildedRose = new GildedRose([
            new ConjuredItem('Mana Cake',4,10)
        ]);
        let items = gildedRose.updateQuality();
        expect(items[0].quality).toBe(8);
    ...
    test('Conjured Item with negative SellIn degrades by four', () => {
        const gildedRose = new GildedRose([
            new ConjuredItem('Mana Cake',0,10)
        ]);
        let items = gildedRose.updateQuality();
        expect(items[0].quality).toBe(6);
        expect(items[0].sellIn).toBe(-1);
...
describe('General Tests for all item types', () => {
    test('For All Items Quality does not go below 0', () => {
        const gildedRose = new GildedRose([
            ...
            new Item('Sulfuras, Hand of Ragnaros',4,80),
            new ConjuredItem('Mana Cake',5,0)
        ]);
        let items = gildedRose.updateQuality();
    ...
    test('For All Items Quality does not go above 50 except Sulfuras', () => {
        const gildedRose = new GildedRose([
            new Item('banana',5,50),
            new Item('Aged Brie',5,50),
            new Item('Backstage passes to a TAFKAL80ETC concert',5,50),
            new ConjuredItem('Mana Cake',5,50)
        ]);
        let items = gildedRose.updateQuality();
        for(const item of items){
